...
const API = {
    ...
    resources: {
        ...
        async uploadToStorage(file) {
            ...
            if (error) {
                console.error('[API.resources.uploadToStorage] upload error', error);
        ...
        async uploadFile(lessonId, file, type = 'file', title = null) {
            const upload = await this.uploadToStorage(file);
            const fileName = file.name.replace(/\.[^/.]+$/, '');
            return this.create({
                lesson_id: lessonId,
                title: title?.trim() || fileName,
                type: type || this._resourceTypeByExt((file.name.split('.').pop() || '').toLowerCase()),
                ...upload
            });
        },
