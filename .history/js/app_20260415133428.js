...
const App = {
            ...
            container.innerHTML = `
                <div class="page-header">
                    <div class="page-title">
                        ...
                        <p>История прохождения курсов и тестов</p>
                    </div>
                </div>

                <div class="stats-grid" style="margin-bottom:2rem">
                    ${[
                        { icon: '📚', label: 'Курсов записан', value: enrollments.length, color: '#6366F1' },
                        { icon: '✅', label: 'Курсов завершено', value: enrollments.filter(e => e.completed_at).length, color: '#10B981' },
                        { icon: '📝', label: 'Попыток тестов', value: attempts.length, color: '#8B5CF6' },
                        { icon: '🏆', label: 'Тестов сдано', value: attempts.filter(a => a.passed).length, color: '#F59E0B' }
                    ].map(s => `
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem" class="dash-two-col">
                    <!-- Course progress -->
                    <div class="card">
                        <div class="card-header"><h3>📚 Мои курсы</h3></div>
                    ...
                    <!-- Test attempts -->
                    <div class="card">
                        <div class="card-header"><h3>📝 Последние тесты</h3></div>
                        <div class="card-body" style="padding:0">
    ...
    async renderProfile(container) {
        UI.setBreadcrumb([{ label: 'Профиль' }]);
        const profile = AppState.profile;

        container.innerHTML = `
            <div style="max-width:600px;margin:0 auto">
                <div class="page-header">
                    <div class="page-title"><h1>👤 Мой профиль</h1></div>
                </div>
                <div class="card">
                    <div class="card-body">
                        <div style="display:flex;align-items:center;gap:1.5rem;margin-bottom:2rem">
                            ...
                            <div>
                                <h2>${profile?.full_name || 'Пользователь'}</h2>
                        ...
                        <button class="btn btn-primary" onclick="App.saveProfile()">Сохранить профиль</button>
                    </div>
                </div>
            </div>`;
    },
