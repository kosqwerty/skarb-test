...
const App = {
    ...
    async renderProfile(container) {
        ...
        container.innerHTML = `
<div style="font-family: -apple-system, BlinkMacSystemFont, 'SF Pro Display', 'SF Pro Text', 'Inter', sans-serif; -webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale; max-width: 900px;margin-left: 30px;">
    ...
    <div style="background: var(--bg-surface); border-radius: 20px; box-shadow: 0 20px 40px rgba(0,0,0,0.3); border: 0.5px solid rgba(255,255,255,0.06); padding: 36px; margin-bottom: 24px; backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px);">
        ...
        <!-- Сетка полей Apple-style -->
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px 40px;">
            ${fieldApple('Логін', profile?.login)}
            ${fieldApple('Телефон', profile?.phone)}
            ${fieldApple('Місто', profile?.city)}
            ${fieldApple('Стать', genderLabel)}
            ${fieldApple('Посада', profile?.job_position)}
            ${fieldApple('Підрозділ', profile?.subdivision)}
            ${fieldApple('Дата народження', profile?.birth_date ? Fmt.date(profile.birth_date) : '')}
            ${fieldApple('Керівник', manager ? `${manager.full_name} ${manager.job_position ? '· ' + manager.job_position : ''}` : '')}
        </div>
    </div>

    <!-- Подчинённые Apple-style -->
    ${subordinates.length ? `
