...
function fieldStyled(label, value) {
    ...
    async editProfile() {
        ...
        container.innerHTML = `
<div style="-webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale; max-width: 720px; margin: 0 auto;">
    ...
    <div style="background: var(--bg-surface); border-radius: 20px; box-shadow: 0 20px 40px rgba(0,0,0,0.3); border: 0.5px solid rgba(255,255,255,0.06); padding: 36px; backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px);">
        ...
        <!-- Поля формы в Apple-стиле -->
        <div style="display: flex; flex-direction: column; gap: 24px;">
            
            <!-- ПІБ + Логін -->
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                ${formField('ПІБ', 'prof-name', 'text', profile?.full_name || '', 'Петренко Тарас')}
                ${formField('Логін', 'prof-login', 'text', profile?.login || '', 'ПризвіщеІм\'я')}
                <div>
                    <label style="display: block; font-size: 13px; font-weight: 500; color: var(--text-secondary); text-transform: uppercase; letter-spacing: 0.02em; margin-bottom: 6px;">Логін</label>
                    <input type="prof-login" value="${profile?.login || ''}" readonly style="width: 100%; padding: 12px 16px; background: rgba(255,255,255,0.02); border: 0.5px solid rgba(255,255,255,0.08); border-radius: 12px; font-size: 15px; font-weight: 400; letter-spacing: -0.01em; color: var(--text-muted); outline: none; opacity: 0.6; cursor: not-allowed; font-family: inherit;">
                </div>
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                ...
                ${formField('Мітка', 'prof-label', 'text', profile?.label || '', 'Наприклад: Техніка, Золото, Валюта')}
            </div>
...
function formField(label, id, type, value, placeholder) {
    return `
        <div>
            ...
            <input id="${id}" type="${type}" value="${value}" placeholder="${placeholder}" style="width: 100%; padding: 12px 16px; background: rgba(255,255,255,0.02); border: 0.5px solid rgba(255,255,255,0.08); border-radius: 12px; font-size: 15px; font-weight: 400; letter-spacing: -0.01em; color: var(--text-primary); outline: none; font-family: inherit; transition: all 0.2s ease;">
        </div>
    `;
}
    },
