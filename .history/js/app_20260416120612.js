        ...
        <div style="display: flex; align-items: center; gap: 28px; margin-bottom: 32px; padding-bottom: 28px; border-bottom: 1px solid var(--border);">
            ...
            <div style="flex: 1;">
                ...
                <div style="display: flex; gap: 8px; flex-wrap: wrap; margin-bottom: 8px;">
                    <span style="background: rgba(99, 102, 241, 0.15); color: var(--primary); padding: 4px 12px; border-radius: var(--radius-full); font-size: 0.8rem; font-weight: 600; letter-spacing: 0.01em; border: 1px solid rgba(99, 102, 241, 0.2);">${Fmt.role(profile?.role)}</span>
...
function fieldStyled(label, value) {
    ...
    async editProfile() {
        ...
        container.innerHTML = `
<div style="-webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale; max-width: 720px; margin: 5px;">
    
    <!-- Шапка  -->
    <div style="margin-bottom: 28px;">
        <h1 style="margin: 0 0 4px; font-weight: 590; font-size: 34px; letter-spacing: -0.022em; color: var(--text-primary); line-height: 1.1;">✎ Редагування профілю</h1>
        <p style="margin: 0; color: var(--text-secondary); font-size: 17px; font-weight: 400; letter-spacing: -0.01em;">Оновіть особисту інформацію</p>
    </div>

    <!-- Форма редактирования -->
    <div style="background: var(--bg-surface); border-radius: 20px; box-shadow: 0 20px 40px rgba(0,0,0,0.3); border: 0.5px solid rgba(255,255,255,0.06); padding: 36px; backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px);">
        ...
        <div style="display: flex; align-items: center; gap: 28px; margin-bottom: 36px; padding-bottom: 28px; border-bottom: 0.5px solid rgba(255,255,255,0.06);">
            <div style="position: relative; flex-shrink: 0;">
                <div id="prof-avatar-preview" style="width: 96px; height: 96px; border-radius: 22px; overflow: hidden; background: linear-gradient(135deg, var(--primary), var(--secondary)); display: flex; align-items: center; justify-content: center; font-size: 36px; font-weight: 500; letter-spacing: -0.5px; color: var(--text-inverse); box-shadow: 0 8px 20px var(--primary-glow); cursor: pointer; transition: all 0.2s cubic-bezier(0.4, 0, 0.2, 1); border: none;" onclick="document.getElementById('prof-avatar-input').click()" title="Змінити фото">
                    ${profile?.avatar_url
                        ? `<img src="${profile.avatar_url}" style="width:100%;height:100%;object-fit:cover">`
                        : Fmt.initials(profile?.full_name)}
                </div>
            ...
            <div>
                <p style="margin: 0 0 8px; font-size: 15px; font-weight: 400; letter-spacing: -0.01em; color: var(--text-secondary);">Натисніть на фото щоб змінити</p>
        ...
        <div style="display: flex; flex-direction: column; gap: 24px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                ${formField('ПІБ', 'prof-name', 'text', profile?.full_name || '', 'Петренко Тарас')}
                ${formField('Логін', 'prof-login', 'text', profile?.login || '', 'ПризвіщеІм\'я', true)}
                
            </div>

            <!-- Телефон + Email -->
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                ${formField('Телефон', 'prof-phone', 'tel', profile?.phone || '', '+380XXXXXXXXX')}
                <div>
                    <label style="display: block; font-size: 13px; font-weight: 500; color: var(--text-secondary); text-transform: uppercase; letter-spacing: 0.02em; margin-bottom: 6px;">Email</label>
                    <input type="email" value="${profile?.email || ''}" readonly style="width: 100%; padding: 12px 16px; background: rgba(255,255,255,0.02); border: 0.5px solid rgba(255,255,255,0.08); border-radius: 12px; font-size: 15px; font-weight: 400; letter-spacing: -0.01em; color: var(--text-muted); outline: none; opacity: 0.6; cursor: not-allowed; font-family: inherit;">
                </div>
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                <div>
                    <label style="display: block; font-size: 13px; font-weight: 500; color: var(--text-secondary); text-transform: uppercase; letter-spacing: 0.02em; margin-bottom: 6px;">Місто</label>
                    <select id="prof-city" style="width: 100%; padding: 12px 16px; background: rgba(255,255,255,0.02); border: 0.5px solid rgba(255,255,255,0.08); border-radius: 12px; font-size: 15px; font-weight: 400; letter-spacing: -0.01em; color: var(--text-primary); outline: none; cursor: pointer; font-family: inherit; transition: all 0.2s ease;">
                        ${opt(cities, profile?.city)}
                    </select>
                </div>
                <div>
                    <label style="display: block; font-size: 13px; font-weight: 500; color: var(--text-secondary); text-transform: uppercase; letter-spacing: 0.02em; margin-bottom: 6px;">Стать</label>
                    <select id="prof-gender" style="width: 100%; padding: 12px 16px; background: rgba(255,255,255,0.02); border: 0.5px solid rgba(255,255,255,0.08); border-radius: 12px; font-size: 15px; font-weight: 400; letter-spacing: -0.01em; color: var(--text-primary); outline: none; cursor: pointer; font-family: inherit; transition: all 0.2s ease;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                <div>
                    <label style="display: block; font-size: 13px; font-weight: 500; color: var(--text-secondary); text-transform: uppercase; letter-spacing: 0.02em; margin-bottom: 6px;">Посада</label>
                    <input id="prof-job-position" type="text" value="${profile?.job_position || ''}" readonly disabled style="width: 100%; padding: 12px 16px; background: rgba(255,255,255,0.02); border: 0.5px solid rgba(255,255,255,0.08); border-radius: 12px; font-size: 15px; font-weight: 400; letter-spacing: -0.01em; color: var(--text-muted); outline: none; opacity: 0.6; cursor: not-allowed; font-family: inherit; transition: all 0.2s ease;">
                </div>
                <div>
                    <label style="display: block; font-size: 13px; font-weight: 500; color: var(--text-secondary); text-transform: uppercase; letter-spacing: 0.02em; margin-bottom: 6px;">Підрозділ</label>
                    <input id="prof-subdivision" type="text" value="${profile?.subdivision || ''}" readonly disabled style="width: 100%; padding: 12px 16px; background: rgba(255,255,255,0.02); border: 0.5px solid rgba(255,255,255,0.08); border-radius: 12px; font-size: 15px; font-weight: 400; letter-spacing: -0.01em; color: var(--text-muted); outline: none; opacity: 0.6; cursor: not-allowed; font-family: inherit; transition: all 0.2s ease;">
                </div>
            </div>

            <!-- Дата народження + Мітка -->
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                <div>
                    <label style="display: block; font-size: 13px; font-weight: 500; color: var(--text-secondary); text-transform: uppercase; letter-spacing: 0.02em; margin-bottom: 6px;">Дата народження</label>
                    <input id="prof-birthdate" type="date" value="${profile?.birth_date || ''}" style="width: 100%; padding: 12px 16px; background: rgba(255,255,255,0.02); border: 0.5px solid rgba(255,255,255,0.08); border-radius: 12px; font-size: 15px; font-weight: 400; letter-spacing: -0.01em; color: var(--text-primary); outline: none; font-family: inherit; transition: all 0.2s ease;">
                </div>
                ${formField('Мітка', 'prof-label', 'text', profile?.label || '', 'Наприклад: Техніка, Золото, Валюта', true)}
            </div>

            <!-- Керівник -->
            <div>
                <label style="display: block; font-size: 13px; font-weight: 500; color: var(--text-secondary); text-transform: uppercase; letter-spacing: 0.02em; margin-bottom: 6px;">Керівник</label>
                <input id="prof-manager" type="text" value="${(allUsers.find(u => u.id === profile?.manager_id)?.full_name || '')}" readonly disabled style="width: 100%; padding: 12px 16px; background: rgba(255,255,255,0.02); border: 0.5px solid rgba(255,255,255,0.08); border-radius: 12px; font-size: 15px; font-weight: 400; letter-spacing: -0.01em; color: var(--text-muted); outline: none; opacity: 0.6; cursor: not-allowed; font-family: inherit; transition: all 0.2s ease;">
                <input type="hidden" id="prof-manager-id" value="${profile?.manager_id || ''}">
            </div>

            <!-- Кнопки действий -->
            <div style="display: flex; gap: 12px; margin-top: 12px; padding-top: 8px;">
                <button class="btn btn-primary" onclick="App.saveProfile()" style="padding: 12px 24px; font-weight: 500; letter-spacing: -0.01em; border-radius: 12px; font-size: 15px;">Зберегти зміни</button>
                <button class="btn btn-secondary" onclick="App.renderProfile(document.getElementById('page-content'))" style="padding: 12px 24px; font-weight: 500; letter-spacing: -0.01em; border-radius: 12px; font-size: 15px;">Скасувати</button>
            </div>
        </div>
    </div>
</div>`;

// Вспомогательная функция для полей ввода 
function formField(label, id, type, value, placeholder, readOnly = false) {
    const attrs = readOnly ? 'readonly disabled' : '';
    const style = readOnly ? 'opacity: 0.6; cursor: not-allowed; color: var(--text-muted);' : 'color: var(--text-primary);';
    return `
        <div>
            <label style="display: block; font-size: 13px; font-weight: 500; color: var(--text-secondary); text-transform: uppercase; letter-spacing: 0.02em; margin-bottom: 6px;">${label}</label>
            <input id="${id}" type="${type}" value="${value}" placeholder="${placeholder}" ${attrs} style="width: 100%; padding: 12px 16px; background: rgba(255,255,255,0.02); border: 0.5px solid rgba(255,255,255,0.08); border-radius: 12px; font-size: 15px; font-weight: 400; letter-spacing: -0.01em; ${style} outline: none; font-family: inherit; transition: all 0.2s ease;">
        </div>
    `;
}
    },
    ...
    async saveProfile() {
        ...
        try {
            const updated = await API.profiles.update(AppState.user.id, {
                ...
                manager_id:   Dom.val('prof-manager-id') || null
            });
