            ...
            <tr id="urow-${u.id}">
                <td>
                    <div style="display:flex;align-items:center;gap:.75rem">
                        <div style="width:32px;height:32px;border-radius:50%;background:linear-gradient(135deg,var(--primary),var(--secondary));display:flex;align-items:center;justify-content:center;font-size:.75rem;font-weight:700;color:#fff;flex-shrink:0;overflow:hidden">
        ...
        Modal.open({
            ...
            body: `
                ...
                <div class="form-group">
                    ...
                    <select id="cu-manager">
                        <option value="">— не вказано —</option>
                        ${users.map(u => `<option value="${u.id}">${u.full_name}${u.job_position ? ' · ' + u.job_position : ''}</option>`).join('')}
                    </select>
                </div>
                <div class="form-group">
                    <label>Мітка</label>
    ...
    async openEditUser(user) {
        ...
        Modal.open({
            ...
            body: `
                ...
                <div class="form-group">
                    <label>Керівник</label>
                    <select id="eu-manager">
                        <option value="">— не вказано —</option>
                        ${users.filter(u => u.id !== user.id).map(u => `<option value="${u.id}" ${u.id === user.manager_id ? 'selected' : ''}>${u.full_name}${u.job_position ? ' · ' + u.job_position : ''}</option>`).join('')}
                    </select>
                </div>
                <div class="form-group">
