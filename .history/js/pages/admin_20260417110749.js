                ...
                <table>
                    <thead>
                        <tr class="filter-row">
                            ...
                            <th>
                                <select id="uf-status" onchange="AdminPage._applyUserFilters()">
            ...
            <tr id="urow-${u.id}"
                ...
                <td>
                    <span class="badge ${u.is_active !== false ? 'badge-success' : 'badge-muted'}">
                        ${u.is_active !== false ? 'Активний' : 'Заблок.'}
                    </span>
                </td>
                <td>
                    <div style="display:flex;gap:.3rem">
                        ...
                        ${AppState.isOwner() && !isSelf && !isOwnerRow ? `
                            <button class="btn btn-ghost btn-sm" title="Передати права власника" onclick="AdminPage.transferOwnership('${u.id}','${(u.full_name||'').replace(/'/g,"\\'")}')">👑</button>` : ''}
                    </div>
                </td>
            </tr>`;
    },
        ...
        try {
            ...
            if (data.user) {
                ...
                await supabase.from('profiles').update({
                    ...
                    login:        Dom.val('cu-login').trim() || null,
            ...
            if (adminSession.data.session &&
                supabase.auth.getUser && (await supabase.auth.getUser()).data?.user?.id !== AppState.user.id) {
    ...
    importUsers() {
        Modal.open({
            ...
            body: `
                ...
                <label style="display:flex;flex-direction:column;align-items:center;gap:.75rem;border:2px dashed var(--border);border-radius:var(--radius-md);padding:2rem;cursor:pointer;transition:border-color .2s"
