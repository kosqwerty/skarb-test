...
const CollectionsPage = {
    ...
    async init(container) {
        ...
        if (!AppState.isStaff()) {
            ...
            try {
                ...
                if (home) {
                    Router.go(`collections/Сторінка${home.id}`);
                } else {
    ...
    async _loadList() {
        ...
        try {
            const pages = await API.pages.getAll();
            const visible = AppState.isStaff() ? pages : pages.filter(p => p.is_published);
            if (!visible.length) {
                el.innerHTML = `
    ...
    _renderCard(p) {
        ...
        const adminBtns = AppState.isStaff() ? `
            <div style="display:flex;gap:.4rem" onclick="event.stopPropagation()">
                ${!p.is_home ? `<button onclick="CollectionsPage.setHome('${p.id}')"
    ...
    async initView(container, { id } = {}) {
        ...
        try {
            ...
            UI.setBreadcrumb([{ label: 'Меню порталу', link: 'collections' }, { label: page.title }]);
        ...
        container.innerHTML = `
            <div style="display:flex;flex-direction:column;gap:1rem">
                ...
                <div id="page-rendered" style="width:fit-content;max-width:100%">
                    <iframe id="page-iframe" style="width:100%;max-width:100%;border:none;display:block" scrolling="no"></iframe>
                </div>
            </div>`;

        // Intercept messages from iframe
        this._navHandler = e => {
            if (e.data?.type === 'lms-navigate') Router.go(e.data.route);
...
(function() {
  ...
  document.addEventListener('DOMContentLoaded', function() { sendSize(0); });
  ...
  ${css || ''}
    ...
    async _openPreviewModal() {
        ...
        box.innerHTML = `
            ...
            <iframe id="preview-modal-iframe" style="flex:1;border:none;background:#fff"></iframe>`;
