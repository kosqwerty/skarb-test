...
const LessonViewPage = {
        ...
        viewer.innerHTML = `
            <div class="card">
                ...
                <div class="card-body" style="padding:0">
                    <object data="${viewerUrl}" type="application/pdf" width="100%" height="700px">
                        <p>Ваш браузер не підтримує вбудований перегляд PDF. <a href="${url}" target="_blank">Відкрити PDF у новій вкладці</a>.</p>
                    </object>
                </div>
                <div style="padding:1rem;text-align:center">
                    <a href="${url}" target="_blank" class="btn btn-primary">Відкрити PDF напряму</a>
                    <p style="margin:.75rem 0 0;color:var(--text-muted)">Якщо перегляд не працює, відкрийте файл у новій вкладці.</p>
                </div>
            </div>`;
    },
    ...
    async saveResource() {
        ...
        const defaultTitle = this._resFile ? this._resFile.name.replace(/\.[^/.]+$/, '') : '';
        const finalTitle = title || defaultTitle;
        if (!finalTitle) { Toast.error('Помилка', 'Вкажіть назву'); return; }
        ...
        try {
            if (type === 'scorm' && this._resFile) {
                ...
                await API.resources.uploadFile(lessonId, this._resFile, type, finalTitle);
            } else if (url) {
                await API.resources.create({ lesson_id: lessonId, title: finalTitle, type, url });
            } else {
