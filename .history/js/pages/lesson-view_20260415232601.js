        ...
        viewer.innerHTML = `
            <div class="card">
                <div class="card-header" style="justify-content:space-between;align-items:flex-start">
                    <div>
                        <h3>📄 ${resource.title}</h3>
                        ${resource.description ? `<p style="margin:.3rem 0 0;color:var(--text-muted)">${resource.description}</p>` : ''}
                    </div>
                    <div style="display:flex;gap:.5rem;flex-wrap:wrap">
                        <a href="${url}" target="_blank" class="btn btn-ghost btn-sm">Відкрити у новому вікні</a>
                        <a href="${url}" target="_blank" class="btn btn-secondary btn-sm">Завантажити ↓</a>
                    </div>
                </div>
