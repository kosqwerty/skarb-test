...
const NewsPage = {
    ...
    _openForm(news) {
        Modal.open({
            title: news ? '✏️ Редагувати новину' : '+ Нова новина',
            size: 'xl',
            body: `
                <div class="form-row">
                    <div class="form-group" style="grid-column:1/-1">
                        <label>Заголовок *</label>
                        <input id="n-title" type="text" value="${news?.title || ''}" placeholder="Заголовок новини">
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select id="n-category">
                            ${['general','announcements','updates','events'].map(c =>
                                `<option value="${c}" ${news?.category === c ? 'selected' : ''}>${{general:'Загальні',announcements:'Оголошення',updates:'Оновлення',events:'Події'}[c]}</option>`
                            ).join('')}
                        </select>
                    </div>
                    <div class="form-group">
                ...
                <div class="form-group">
                    <label>Опис для прев'ю</label>
                    <textarea id="n-excerpt" style="min-height:60px" placeholder="Короткий опис...">${news?.excerpt || ''}</textarea>
                </div>
                <div class="form-group">
                    <label>Зміст *</label>
            ...
            footer: `
                <button class="btn btn-secondary" onclick="Modal.close()">Відміна</button>
                <button class="btn btn-primary" onclick="NewsPage.saveNews('${news?.id || ''}')">Збергіти</button>`
        });
        ...
        setTimeout(() => {
            ...
            if (zone) {
                const input = FileUpload.createDropZone(zone, { accept: 'image/*', label: 'Додати обкладинку', hint: 'PNG, JPG до 5 МБ' });
    ...
    async saveNews(id) {
        ...
        if (!content || content === '<p><br></p>') { Toast.error('Помилка', 'Додайте зміст'); return; }
        ...
        try {
            ...
            Toast.success('Уря!', `Новина "${title}" ${id ? 'оновлена' : 'додана'}`);
            Router.go('news');
        } catch(e) {
