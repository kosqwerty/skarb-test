...
const ResourcesPage = {
    ...
    _buildViewerContent(resource, url) {
        ...
        if (resource.type === 'pdf' || ext === 'pdf') {
            ...
            return `${description}
                <object data="${viewerUrl}" type="application/pdf" width="100%" height="75vh">
                    <p>Ваш браузер не підтримує вбудований перегляд PDF. <a href="${url}" target="_blank">Відкрити PDF у новій вкладці</a>.</p>
                </object>
                <div style="margin-top:1rem;text-align:center">
                    <a href="${url}" target="_blank" class="btn btn-primary">Відкрити в новому вікні</a>
                    <span style="display:block;margin-top:.75rem;color:var(--text-muted)">Якщо перегляд не працює, відкрийте файл напряму.</span>
                </div>`;
        }
        ...
        if (['doc','docx','xls','xlsx','ppt','pptx'].includes(ext)) {
            const viewerUrl = this._officeViewerUrl(url);
            return `${description}<iframe src="${viewerUrl}" style="width:100%;height:75vh;border:none"></iframe>`;
        }

        if (['txt','csv'].includes(ext)) {
            return `${description}<iframe src="${url}" style="width:100%;height:75vh;border:none"></iframe>`;
        }
    ...
    async downloadResource(id) {
        ...
        try {
            const resource = await API.resources.getById(id);
            const url = await this._getResourceUrl(resource);
            const fileName = resource.title || resource.storage_path?.split('/').pop() || 'resource';
    ...
    openForm(resource = null) {
        ...
        if (uploadContainer) {
            FileUpload.createDropZone(uploadContainer, {
                accept: '*/*',
    ...
    async saveResource(resourceId) {
        ...
        try {
            if (resourceId) {
                ...
                await API.resources.createWithFile(this._resourceFile, fields);
