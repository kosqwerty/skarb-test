...
const ResourcesPage = {
    ...
    _buildViewerContent(resource, url) {
        ...
        if (resource.type === 'pdf' || ext === 'pdf') {
            const viewerUrl = `pdf-viewer.html?file=${encodeURIComponent(url)}&title=${encodeURIComponent(resource.title || 'PDF')}`;
            return `${description}<iframe src="${viewerUrl}"   style="width:100%;height:75vh;border:none"   sandbox="allow-scripts allow-same-origin"></iframe>`;
        }
