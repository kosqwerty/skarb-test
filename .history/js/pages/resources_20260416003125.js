...
const ResourcesPage = {
    ...
    openViewer(id) {
        const from = this._view === 'admin' ? 'resources' : 'knowledge-base';
...
const ResourceViewPage = {
    ...
    async init(container, { id, from = 'knowledge-base' } = {}) {
        if (!id) { Router.go(from); return; }

        UI.setBreadcrumb([
            { label: from === 'resources' ? 'Ресурси' : 'База знань', link: from },
            { label: 'Перегляд' }
        ]);
        ...
        try {
            ...
            this._render(container, resource, url, from);
        } catch (e) {
            container.innerHTML = `
                <div class="empty-state">
                    <div class="empty-icon">⚠️</div>
                    <h3>${e.message}</h3>
                    <button class="btn btn-primary" onclick="Router.go('${from}')">← Назад</button>
                </div>`;
        }
    },
    ...
    _render(container, resource, url, from) {
        const ext = resource.storage_path
            ? resource.storage_path.split('.').pop().toLowerCase()
            : (resource.file_type?.split('/').pop() || '');
        ...
        if (isPdf) {
            ...
            viewerHtml = `
                <div style="background:#000;border-radius:var(--radius-lg);overflow:hidden">
                    <video controls src="${url}" style="width:100%;max-height:calc(100vh - 240px);display:block"></video>
                </div>`;
        ...
        container.innerHTML = `
            <div style="display:flex;flex-direction:column;gap:1rem">
                ...
                <div style="display:flex;align-items:flex-start;gap:1rem;flex-wrap:wrap">
                    <button class="btn btn-ghost btn-sm" onclick="Router.go('${from}')" style="flex-shrink:0;margin-top:.2rem">
                        ← Назад
                    </button>
                    <div style="flex:1;min-width:0">
                        <h1 style="margin:0 0 .4rem;font-size:1.4rem;font-weight:700;line-height:1.3">${resource.title}</h1>
                    ...
                    <button class="btn btn-secondary btn-sm" style="flex-shrink:0"
                            onclick="ResourcesPage.downloadResource('${resource.id}')">
                        ⬇ Скачати
                    </button>` : ''}
                </div>
