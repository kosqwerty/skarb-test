...
const Utils = {
    pdfViewerUrl(fileUrl, title = 'PDF Viewer') {
        if (!fileUrl) return '';
        const absoluteFileUrl = new URL(fileUrl, window.location.href).toString();
        const viewerUrl = new URL('pdf-viewer.html', window.location.href);
        viewerUrl.searchParams.set('file', absoluteFileUrl);
        viewerUrl.searchParams.set('title', title || 'PDF Viewer');
