...
CREATE POLICY IF NOT EXISTS "lesson_resources_objects_insert" ON storage.objects FOR INSERT TO authenticated
    WITH CHECK (bucket_id = 'lesson-resources' AND auth.uid() IS NOT NULL);
CREATE POLICY IF NOT EXISTS "lesson_resources_objects_select" ON storage.objects FOR SELECT TO authenticated
    USING (bucket_id = 'lesson-resources' AND auth.uid() IS NOT NULL);
CREATE POLICY IF NOT EXISTS "lesson_resources_objects_update" ON storage.objects FOR UPDATE TO authenticated
    USING (bucket_id = 'lesson-resources' AND auth.uid() IS NOT NULL);
CREATE POLICY IF NOT EXISTS "lesson_resources_objects_delete" ON storage.objects FOR DELETE TO authenticated
    USING (bucket_id = 'lesson-resources' AND auth.uid() IS NOT NULL);
