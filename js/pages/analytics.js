...
const AnalyticsPage = {
    ...
    async init(container, params) {
        ...
        UI.setBreadcrumb([{ label: 'Аналітика' }]);

        container.innerHTML = `
            <div class="page-header">
                <div class="page-title">
                    <h1>📊 Аналітика</h1>
                    <p>Статистика навчання та прогрес</p>
                </div>
                <div class="page-actions">
                    <select id="analytics-course" onchange="AnalyticsPage.loadCourseStats()" style="width:auto">
                        <option value="">Усі курси</option>
                    </select>
                    <button class="btn btn-success" onclick="AnalyticsPage.exportAll()">📊 Експорт Excel</button>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-header">
                    <h3>📝 Спроб тестів</h3>
                    <button class="btn btn-success btn-sm" onclick="AnalyticsPage.exportAttempts()">Експорт</button>
                </div>
    ...
    _renderStats(stats) {
        ...
        el.innerHTML = [
            { icon: '👥', label: 'Усього користувачів', value: Fmt.num(stats.totalUsers), color: '#6366F1' },
            { icon: '🎓', label: 'Усього записів',       value: Fmt.num(stats.totalEnrollments), color: '#8B5CF6' },
            { icon: '✅', label: 'Завершено курсів',    value: Fmt.num(completed), color: '#10B981' },
            { icon: '📝', label: 'Спроби тестів',      value: Fmt.num(stats.totalAttempts), color: '#06B6D4' },
            { icon: '🏆', label: 'Успішно тестів',        value: passRate + '%', color: '#F59E0B' },
            { icon: '💯', label: 'Середній бал',         value: avgScore + '%', color: '#EF4444' }
        ].map(s => `
    ...
    _renderStudentRows(rows, el) {
        ...
        el.innerHTML = `
            <table>
                <thead>
                    <tr>
                        <th>Користувач</th><th>Email</th><th>Курсов</th><th>Завершено</th><th>Ср. прогрес</th>
                    </tr>
                </thead>
                <tbody>
                    ${rows.map(r => {
    ...
    _renderAttemptsTable(attempts) {
        ...
        el.innerHTML = `
            <table>
                <thead>
                    <tr>
                        <th>Стажер</th><th>Тест</th><th>Курс</th><th>Результат</th><th>Статус</th><th>Время</th><th>Дата</th>
                    </tr>
                </thead>
                <tbody>
    ...
    exportStudents() {
        ...
        Excel.export(rows, 'students_analytics', 'Стажери');
    },

    exportAttempts() {
        const rows = (this._attemptRows || []).map(a => ({
            'Стажер':   a.user?.full_name,
            'Email':    a.user?.email,
            'Тест':     a.test?.title,
