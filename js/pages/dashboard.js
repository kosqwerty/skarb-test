...
const DashboardPage = {
    async init(container) {
        ...
        const [stats, enrollments, news, birthdays] = await Promise.all([
    ...
    _renderBirthdays(people) {
        ...
        el.innerHTML = `
            <div style="background:linear-gradient(135deg,#f59e0b22,#f59e0b11);border:1px solid #f59e0b44;border-radius:var(--radius);padding:1rem 1.25rem;margin-bottom:1.5rem;display:flex;align-items:center;gap:1rem;flex-wrap:wrap">
                <span style="font-size:2rem">🎂</span>
                <div style="flex:1;min-width:0">
                    <div style="font-weight:600;color:#f59e0b;margin-bottom:.25rem">Сьогодні день народження!</div>
                    <div style="display:flex;flex-wrap:wrap;gap:.5rem">
                        ${people.map(p => `
                            <span style="background:var(--bg-raised);border:1px solid var(--border);border-radius:var(--radius-sm);padding:.2rem .65rem;font-size:.85rem">
                                🎉 <strong>${p.full_name}</strong>${p.job_position ? ` · ${p.job_position}` : ''}${p.subdivision ? ` · ${p.subdivision}` : ''}
                            </span>`).join('')}
                    </div>
                </div>
            </div>`;
